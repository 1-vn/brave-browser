...
const Config = function () {
  ...
  this.gClientFile = process.env.ONEVN_GCLIENT_FILE || this.defaultGClientFile
  this.gClientVerbose = getNPMConfig(['gclient_verbose']) || false
  this.targetArch = 'x64'
  this.gypTargetArch = 'x64'
  this.officialBuild = true
  this.debugBuild = JSON.parse(getNPMConfig(['onevn_debug_build']) || false)
  this.onevnGoogleApiKey = getNPMConfig(['onevn_google_api_key']) || 'AIzaSyAQfxPJiounkhOjODEO5ZieffeBv6yft2Q'
  this.googleApiKey = getNPMConfig(['google_api_key']) || 'AIzaSyAH90V94EcZBP5oH7oc-mXQrSKgASVxER8'
  this.googleApiEndpoint = getNPMConfig(['onevn_google_api_endpoint']) || 'https://www.googleapis.com/geolocation/v1/geolocate?key='
  this.safeBrowsingApiEndpoint = getNPMConfig(['safe_browsing_api_endpoint']) || 'safebrowsing.1-vn.com'
  this.buildProjects()
  this.onevnVersion = getNPMConfig(['version']) || '0.0.0.0'
  this.chromeVersion = getNPMConfig(['projects', 'chrome', 'tag']) || '0.0.0.0'
  this.releaseTag = this.onevnVersion.split('+')[0]
  ...
  this.onevnReferralsApiKey = getNPMConfig(['onevn_referrals_api_key']) || ''
...
Config.prototype.buildArgs = function () {
  const version = this.onevnVersion
  ...
  let args = {
    ...
    enable_nacl: false,
    // branding_path_component: "onevn",
    enable_widevine: true,
    target_cpu: this.targetArch,
    is_official_build: this.officialBuild,
    is_debug: this.buildConfig !== 'Release',
    dcheck_always_on: this.buildConfig !== 'Release',
    onevn_channel: this.channel,
    google_api_key: this.googleApiKey,
    onevn_google_api_key: this.onevnGoogleApiKey,
    onevn_google_api_endpoint: this.googleApiEndpoint,
    onevn_product_name: getNPMConfig(['onevn_product_name']) || "onevn-core",
    onevn_project_name: getNPMConfig(['onevn_project_name']) || "onevn-core",
    onevn_version_major: version_parts[0],
    onevn_version_minor: version_parts[1],
    onevn_version_build: version_parts[2],
    ...
    onevn_referrals_api_key: this.onevnReferralsApiKey,
  ...
  if (process.platform === 'win32') {
    args.cc_wrapper = path.join(this.srcDir, 'onevn', 'script', 'redirect-cc.cmd')
  } else {
    args.cc_wrapper = path.join(this.srcDir, 'onevn', 'script', 'redirect-cc.py')
  }
  return args
}
...
Config.prototype.update = function (options) {
  ...
  if (options.onevn_google_api_key) {
    this.onevnGoogleApiKey = options.onevn_google_api_key
  }

  if (options.onevn_google_api_endpoint) {
    this.googleApiEndpoint = options.onevn_google_api_endpoint
  }
  ...
  if (options.onevn_referrals_api_key) {
    this.onevnReferralsApiKey = options.onevn_referrals_api_key
  }
...
Object.defineProperty(Config.prototype, 'defaultOptions', {
  get: function () {
    ...
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'onevn', 'chromium_src', 'python_modules'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'onevn', 'script'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'tools', 'grit', 'grit', 'extern'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'onevn', 'vendor', 'requests'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'build'))
    env.GCLIENT_FILE = this.gClientFile
    env.DEPOT_TOOLS_WIN_TOOLCHAIN = '0'
    env.PYTHONUNBUFFERED = '1'
    env.TARGET_ARCH = this.gypTargetArch // for onevn scripts
