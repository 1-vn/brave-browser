...
const touchOverriddenFiles = () => {
  ...
  const chromiumSrcDir = path.join(config.srcDir, 'onevn', 'chromium_src')
...
const touchOverriddenVectorIconFiles = () => {
  console.log('touch original vector icon files overridden by onevn/vector_icons...')
  ...
  const onevnVectorIconsDir = path.join(config.srcDir, 'onevn', 'vector_icons')
  var onevnVectorIconFiles = util.walkSync(onevnVectorIconsDir, applyFileFilter)

  // Touch original files by updating mtime.
  const onevnVectorIconsDirLen = onevnVectorIconsDir.length
  onevnVectorIconFiles.forEach(onevnVectorIconFile => {
    var overriddenFile = path.join(config.srcDir, onevnVectorIconFile.slice(onevnVectorIconsDirLen))
    if (fs.existsSync(overriddenFile)) {
      // If overriddenFile is older than file in vector_icons, touch it to trigger rebuild.
      if (fs.statSync(onevnVectorIconFile).mtimeMs - fs.statSync(overriddenFile).mtimeMs > 0) {
        const date = new Date()
        fs.utimesSync(overriddenFile, date, date)
        console.log(overriddenFile + ' is touched.')
      }
...
/**
 * Checks to make sure the src/chrome/VERSION matches onevn-browser's package.json version
 */
const checkVersionsMatch = () => {
  ...
  const onevnVersionFromChromeFile = `${found[2]}.${found[3]}.${found[4]}`
  if (onevnVersionFromChromeFile !== config.onevnVersion) {
    console.warn(`Version files do not match!\nsrc/chrome/VERSION: ${onevnVersionFromChromeFile}\nonevn-browser package.json version: ${config.onevnVersion}`)
