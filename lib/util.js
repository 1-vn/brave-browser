...
const autoGeneratedOneVNToChromiumMapping = Object.assign({}, require('./l10nUtil').autoGeneratedOneVNToChromiumMapping)
...
const util = {
  ...
  updateBranding: () => {
    ...
    const onevnComponentsDir = path.join(config.projects['onevn-core'].dir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const onevnAppDir = path.join(config.projects['onevn-core'].dir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const onevnBrowserResourcesDir = path.join(config.projects['onevn-core'].dir, 'browser', 'resources')
    const onevnAppVectorIconsDir = path.join(config.projects['onevn-core'].dir, 'vector_icons', 'chrome', 'app')

    let fileMap = new Set();
    // The following 3 entries we map to the same name, not the chromium equivalent name for copying back
    autoGeneratedOneVNToChromiumMapping[path.join(onevnAppDir, 'onevn_strings.grd')] = path.join(chromeAppDir, 'onevn_strings.grd')
    autoGeneratedOneVNToChromiumMapping[path.join(onevnAppDir, 'settings_onevn_strings.grdp')] = path.join(chromeAppDir, 'settings_onevn_strings.grdp')
    autoGeneratedOneVNToChromiumMapping[path.join(onevnAppDir, 'components_onevn_strings.grd')] = path.join(config.srcDir, 'components', 'components_onevn_strings.grd')
    Object.entries(autoGeneratedOneVNToChromiumMapping).forEach(mapping => fileMap.add(mapping))

    // Copy xtb files for:
    // onevn/app/resources/chromium_strings*.xtb
    // onevn/app/strings/components_chromium_strings*.xtb
    // onevn/app/resources/generated_resoruces*.xtb
    fileMap.add([path.join(onevnAppDir, 'resources'), path.join(chromeAppDir, 'resources')])
    fileMap.add([path.join(onevnAppDir, 'strings'), path.join(chromeComponentsDir, 'strings')])
    fileMap.add([path.join(onevnAppDir, 'theme', 'default_100_percent', 'onevn'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'onevn')])
    fileMap.add([path.join(onevnAppDir, 'theme', 'default_200_percent', 'onevn'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'onevn')])
    // By overwriting, we don't need to modify some grd files.
    fileMap.add([path.join(onevnAppDir, 'theme', 'onevn'), path.join(chromeAppDir, 'theme', 'onevn')])
    fileMap.add([path.join(onevnAppDir, 'theme', 'onevn'), path.join(chromeAppDir, 'theme', 'chromium')])
    fileMap.add([path.join(onevnAppDir, 'theme', 'default_100_percent', 'onevn'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(onevnAppDir, 'theme', 'default_200_percent', 'onevn'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(onevnComponentsDir, 'resources', 'default_100_percent', 'onevn'), path.join(chromeComponentsDir, 'resources', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(onevnComponentsDir, 'resources', 'default_200_percent', 'onevn'), path.join(chromeComponentsDir, 'resources', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(onevnAppVectorIconsDir, 'vector_icons', 'onevn'), path.join(chromeAppDir, 'vector_icons', 'onevn')])
    // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with onevn's on Win8.
    fileMap.add([path.join(onevnBrowserResourcesDir, 'chrome-logo-faded.png'), path.join(chromeBrowserResourcesDir, 'chrome-logo-faded.png')])
    // Copy to make our ${branding_path_component}_behaviors.cc
    fileMap.add([path.join(config.projects['onevn-core'].dir, 'chromium_src', 'chrome', 'installer', 'setup', 'onevn_behaviors.cc'),
                 path.join(config.srcDir, 'chrome', 'installer', 'setup', 'onevn_behaviors.cc')])
    ...
    if (process.platform === 'darwin') {
      ...
      // Each channel's app icons are stored in onevn/app/theme/$channel/app.icns.
      // With this copying, we don't need to modify chrome/BUILD.gn for this.
      const iconSource = path.join(onevnAppDir, 'theme', 'onevn', 'mac', config.channel, 'app.icns')
      const iconDest = path.join(chromeAppDir, 'theme', 'onevn', 'mac', 'app.icns')
      ...
      const brandingSource = path.join(onevnAppDir, 'theme', 'onevn', branding_file_name)
      const brandingDest = path.join(chromeAppDir, 'theme', 'onevn', 'BRANDING')
  ...
  updateOmahaMidlFiles: () => {
    ...
    const srcDir = path.join(config.projects['onevn-core'].dir, 'win_build_output', 'midl', 'google_update')
  ...
  // Latest version and download url are inserted to cdm header file and onevn-core refers it.
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.projects['onevn-core'].dir
